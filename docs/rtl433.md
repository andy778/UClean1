...
short answer: the *HCS08* code does not build the on-air bytes — **but the
nRF9E5's 8051 firmware, which does, turns out to be embedded in this same flash**
(U2 serves it to the radio at boot; extracted to `dumps/u1-nrf9e5-8051.bin`, see
[nrf9e5-firmware.md](nrf9e5-firmware.md)). The analysis below still pins down the
architecture and the decode *targets*.

Findings:

- **The SPI link is a custom MC9S08↔8051 message bus — and the nRF9E5's boot
  channel.** SPI1C1 = 0xCC (MSTR=0) — the MC9S08 is the SPI *slave*; the nRF9E5's
  8051 is the master. The HCS08 code never *builds* the nRF905 CONFIG, and the
  on-air framing (`fd7a`, `bababa` sync, the `83` tag, software Manchester) is the
  **8051's** doing — but that 8051 program is **served out of this flash** at boot,
  so it *is* recoverable here (the `0x80bb` blob below), not only from captures.
...
- **Inter-MCU message format** (`FUN_db22`, message types `0x20 0x21 0x22 0x23
  0x26`): each message carries **four live 16-bit variables** from RAM
  `0x013e 0x0140 0x0109 0x010b` plus a 2-byte header at `0x06be/0x06bf`. These are
  the CPU-side state words the MC9S08 reports up to the 8051 — a hint at how many
  telemetry quantities exist, but not their on-air byte offsets.
  *(The `02 00 2b 32 ff ff ff` at flash `0x80bb` is **not** a message — it is the
  8051 **reset vector**, the start of the nRF9E5 boot image; see
  [nrf9e5-firmware.md](nrf9e5-firmware.md).)*
...
**Most unknowns are now resolved** — not from captures but by reading the nRF9E5's
own `W_CONFIG` write out of its 8051 firmware (see
[nrf9e5-firmware.md](nrf9e5-firmware.md), "nRF905 radio configuration"):

| Unknown | Symbol in uponor_clean1.c | Value | Source |
|---------|---------------------------|-------|--------|
| [U1] data rate     | `short_width`/`long_width` | 100 kbps line / 50 kbps Manchester | capture |
| [U2] address width | `UCLEAN1_ADDR_BYTES`       | **4 bytes** (`RX_AFW=4`) | firmware `W_CONFIG` |
| [U3] address bytes | `uclean1_address[]`        | **`EA EA EA EA`** (`W_TX_ADDRESS`) | firmware |
| [U4] CRC width     | `UCLEAN1_CRC_BITS`         | **16-bit, enabled** (`CRC_MODE=1`) | firmware `W_CONFIG` |
| [U5] payload length| `UCLEAN1_PAYLOAD_BYTES`    | **32 bytes** (`RX_PW=0x20`) | firmware + RX loop |
| channel            | (RF, not in decoder)      | **CH 117 → 868.2 MHz** (`HFREQ_PLL=1`) | firmware, matches capture |
| [U6] field meaning | `data_make(...)` + `output_fields[]` | **open** | display capture or 8051 parser |
| [U7] whitening     | (none)                    | none (payload is plaintext) | 0d |
| [U8] bit order     | (MSB-first)               | confirm via CRC match | — |

Note the nRF905 hardware handles the address + CRC-16 itself, so the
Manchester-decoded stream is the 32-byte **payload** (`fd 7a ba ba ba 83` app
header + telemetry); `EAEAEAEA` and the CRC are not visible in it. Only `[U6]`
(payload field semantics) remains — see the RX parser at `0x01c8` in the 8051
image, or run a display-correlated capture.
